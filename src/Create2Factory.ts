import { BigNumberish, BytesLike, concat, ethers, getBytes, hexlify, keccak256, Signer, TransactionRequest, zeroPadBytes } from "ethers";
...
export class Create2Factory {
   ...
   * @param salt
   */
  static getDeployedAddress(initCode: string, salt: BytesLike): string {
    const saltBytes32 = zeroPadBytes(hexlify(salt), 32);
    return (
      "0x" +
      keccak256(
        concat([
  ...
  async deployFactory(): Promise<void> {
    ...
    const tx = await this.signer.sendTransaction({
      ...
      value: Create2Factory.factoryDeploymentFee,
    });
    await tx.wait()
    const tx2 = await this.provider.broadcastTransaction(Create2Factory.factoryTx);
    await tx2.wait()
  ...
  getDeployTransactionCallData (initCode: string, salt: BytesLike = "0x"): string {
    const saltBytes32 = zeroPadBytes(hexlify(salt), 32)
    return concat([
      saltBytes32,
      initCode
